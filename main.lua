...
local uiToggle = uiToggleSec:Toggle({
	...
	Callback = function(v: boolean)
		warn("[misanthropy] Show UI toggle -> " .. tostring(v))
		Window:SetOpen(v)
	end,
})
...
if type(uiToggle.Keybind) == "function" then
	uiToggle:Keybind({
		...
		Callback = function(toggled: boolean)
			warn("[misanthropy] Show UI keybind (native) -> " .. tostring(toggled))
			uiToggle:Set(toggled)
		end,
	})
else
	warn("[misanthropy] nhack Toggle has no :Keybind, using raw hotkey fallback (RightAlt)")
	UserInputService.InputBegan:Connect(function(input: InputObject, gameProcessed: boolean)
		if gameProcessed then return end
		if input.KeyCode == Enum.KeyCode.RightAlt then
			local newState = not uiToggle:Get()
			warn("[misanthropy] Show UI keybind (fallback) -> " .. tostring(newState))
			uiToggle:Set(newState)
		end
	end)
end

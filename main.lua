...
----------------------------------------------------------------------------------
-- SECTION 5z2b: Error Log
----------------------------------------------------------------------------------
-- Catches script errors (and optionally warnings) via LogService.MessageOut,
-- which fires for output from ANY script, not just this one - there's no
-- way to filter it down to "only misanthropy's own errors" since Roblox
-- doesn't hand LogService the source script, only the message text and
-- type. Still the best available way to see what actually went wrong
-- without alt-tabbing to a separate console.
do
	local elSec = newSection(pgUtility, "Error Log")
	local elEnabled = elSec:Toggle({ Name = "Capture errors", Default = true, Flag = "el_enabled" })
	local elWarnings = elSec:Toggle({ Name = "Also capture warnings", Default = false, Flag = "el_warnings" })

	local LogService = game:GetService("LogService")
	local EL_MAX_ENTRIES = 40

	-- chudvision's Label is single-line/AutomaticSize.X only - no wrapping,
	-- no multi-line - so it can't display a log. Section.Items["Content"]
	-- is the real Frame every native widget parents into (confirmed from
	-- chudvision's own Toggle/Slider/etc constructors), so a custom
	-- ScrollingFrame dropped in there sits naturally in the same vertical
	-- stack as the toggles/buttons above and below it.
	local elScroll = Instance.new("ScrollingFrame")
	elScroll.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
	elScroll.BorderSizePixel = 0
	elScroll.Size = UDim2.new(1, 0, 0, 160)
	elScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
	elScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
	elScroll.ScrollBarThickness = 4
	elScroll.Parent = elSec.Items["Content"].Instance
	local elPad = Instance.new("UIPadding")
	elPad.PaddingLeft = UDim.new(0, 6); elPad.PaddingRight = UDim.new(0, 6)
	elPad.PaddingTop = UDim.new(0, 4); elPad.PaddingBottom = UDim.new(0, 4)
	elPad.Parent = elScroll
	local elText = Instance.new("TextLabel")
	elText.BackgroundTransparency = 1
	elText.Size = UDim2.new(1, -4, 0, 0)
	elText.AutomaticSize = Enum.AutomaticSize.Y
	elText.Font = Enum.Font.Code
	elText.TextSize = 12
	elText.TextColor3 = Color3.fromRGB(220, 120, 120)
	elText.TextWrapped = true
	elText.TextXAlignment = Enum.TextXAlignment.Left
	elText.TextYAlignment = Enum.TextYAlignment.Top
	elText.Text = "No errors captured yet."
	elText.Parent = elScroll

	local elEntries: { string } = {}

	local function elRefresh()
		elText.Text = #elEntries > 0 and table.concat(elEntries, "\n\n") or "No errors captured yet."
	end

	local function elAdd(prefix: string, message: string)
		local t = DateTime.now():ToLocalTime()
		local stamp = string.format("%02d:%02d:%02d", t.Hour, t.Minute, t.Second)
		table.insert(elEntries, 1, ("[%s] %s: %s"):format(stamp, prefix, message))
		while #elEntries > EL_MAX_ENTRIES do table.remove(elEntries) end
		elRefresh()
	end

	local elConn = LogService.MessageOut:Connect(function(message: string, msgType: Enum.MessageType)
		if not elEnabled:Get() then return end
		if msgType == Enum.MessageType.MessageError then
			elAdd("ERROR", message)
		elseif msgType == Enum.MessageType.MessageWarning and elWarnings:Get() then
			elAdd("WARN", message)
		end
	end)
	table.insert(cleanups, function() elConn:Disconnect() end)

	elSec:Button({ Name = "Clear log", Callback = function()
		table.clear(elEntries)
		elRefresh()
	end })

	local _setclipboard = (setclipboard or toclipboard or (Clipboard and Clipboard.set)) :: any
	if _setclipboard then
		elSec:Button({ Name = "Copy latest entry", Callback = function()
			if #elEntries == 0 then notify("Error Log", "Nothing to copy."); return end
			pcall(_setclipboard, elEntries[1])
			notify("Error Log", "Copied to clipboard.")
		end })
	end

	CFG.toggles.el_enabled = elEnabled; CFG.toggles.el_warnings = elWarnings
end
